<x-app-layout>
    ...
    <div class="container">
        ...
        <div class="container mx-auto px-4">
            ...
            @endif
            <form method="POST" action="{{ route('update_application') }}">
                @csrf
                @if ($role == 'FK Student')
                    <div>
                        <x-label for="name" value="{{ __('Name') }}" />
                        <x-input id="name" class="block mt-1 w-full" type="text" name="name"
                            value="{{ $users->name }}" required autofocus autocomplete="name" />
                    </div>
                    </br>
                    <div>
                        <x-label for="stdID" value="{{ __('Student ID') }}" />
                        <x-input id="stdID" class="block mt-1 w-full" type="text" name="stdID"
                            value="{{ $users->MatricID }}" required autofocus autocomplete="stdID" />
                    </div>
                    </br>
                    <div>
                        <x-label for="product" value="{{ __('Product Name') }}" />
                        <x-input id="product" class="block mt-1 w-full" type="text" name="product"
                            value="{{ $users->Product_name }}" required autofocus autocomplete="product" />
                    </div>
                    </br>
                    <div>
            ...
            <form x-data="{ tooltip: 'Delete' }" action="{{ route('applications.destroy', ['application' => $users->id]) }}"
